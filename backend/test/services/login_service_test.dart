...
void main() {
  ...
  group('LoginService - Metodo login', () {
    ...
    test('Deve restituire null se l\'utente non esiste', () async {
      ...
      // act
      final result = await loginService.login(email: email, password: 'passwordCorretta123');
    ...
    test('Deve effettuare login con successo usando numero di telefono con prefisso +39', () async {
      // arrange
      ...
      final result = await loginService.login(telefono: telefono, password: password);
    ...
    test('Deve restituire null se il numero di telefono non esiste o ha formato errato', () async {
      // arrange
      // Formato sbagliato o inesistente
      const telefonoErrato = '12345';

      // Il repository restituisce null (non trovato)
      when(mockUserRepository.findUserByPhone(telefonoErrato))
          .thenAnswer((_) async => null);

      // act
      final result = await loginService.login(telefono: telefonoErrato, password: 'password');
