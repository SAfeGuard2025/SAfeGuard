...
void main() {
  ...
  group('VerificationService - Metodo completePhoneVerification', () {
    ...
    test('Deve verificare OTP e aggiornare stato utente se OTP è corretto', () async {
      // arrange
      ...
      when(mockUserRepository.verifyOtp(telefono, otp))
          .thenAnswer((_) async => true);

      // Si simula il recupero dell'utente
      when(mockUserRepository.findUserByPhone(telefono))
          .thenAnswer((_) async => {'email': email, 'nome': 'Test'});

      // Si simula l'aggiornamento dello stato
      when(mockUserRepository.markUserAsVerified(email))
          .thenAnswer((_) async => {});

      // act
      final result = await verificationService.completePhoneVerification(telefono, otp);
    ...
    test('Deve restituire false e NON aggiornare utente se OTP è errato', () async {
      // arrange
      const telefono = '+393331234567';
      const otpErrato = '000000';

      // OTP non valido
      when(mockUserRepository.verifyOtp(telefono, otpErrato))
          .thenAnswer((_) async => false);

      // act
      final result = await verificationService.completePhoneVerification(telefono, otpErrato);
    ...
    test('Deve restituire true ma NON aggiornare nulla se l\'utente non esiste', () async {
      // arrange
